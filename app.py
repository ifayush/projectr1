...
# Function to get relevant links based on the question
def get_relevant_links(question: str) -> List[LinkInfo]:
    links = []
    if "gpt-3.5-turbo-0125" in question.lower():
        links.append(LinkInfo(url="https://discourse.onlinedegree.iitm.ac.in/t/ga5-question-8-clarification/155939", text="GA5 Question 8 Clarification"))
    if "ga4" in question.lower() and "bonus" in question.lower():
        links.append(LinkInfo(url="https://discourse.onlinedegree.iitm.ac.in/t/ga4-data-sourcing-discussion-thread-tds-jan-2025/165959", text="GA4 Data Sourcing Discussion Thread"))
    if "docker" in question.lower() and "podman" in question.lower():
        links.append(LinkInfo(url="https://tds.s-anand.net/#/docker", text="Docker Documentation"))
    if "ga4" in question.lower() and "json" in question.lower():
        links.append(LinkInfo(url="https://discourse.onlinedegree.iitm.ac.in/t/ga4-json-scoring-details/165984", text="GA4 JSON Scoring Details"))
    return links

# Update the query_knowledge_base function to include relevant links
@app.post("/query")
    ...
    try:
        ...
        try:
            ...
            if not similar_results:
                logger.warning("No similar content found")
                return QueryResponse(
                    answer="I couldn't find any relevant information in the knowledge base to answer your question. Please try rephrasing your question or contact the course staff for assistance.",
                    links=[]
                )
            ...
            answer_response = await generate_answer(request.question, enriched_results)
            
            # Parse the response
            parsed_response = parse_llm_response(answer_response)
            
            # Get relevant links based on the question
            relevant_links = get_relevant_links(request.question)
            
            # Combine parsed links with relevant links
            all_links = relevant_links + [LinkInfo(url=link["url"], text=link["text"]) for link in parsed_response["links"]]
